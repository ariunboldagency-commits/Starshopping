...
const ORIGIN = 'https://starshopping-mn.github.io';
(async () => {
  ...
  await page.route(/https:\/\/(starshopping-mn\.github\.io|script\.google\.com|script\.googleusercontent\.com|drive\.google\.com)\//, async route => { try { const r = await route.fetch({ maxRedirects: 5 }); await route.fulfill({ response: r, body: await r.body() }); } catch (e) { try { await route.abort(); } catch {} } });
